...
export type BlazeFeedBackStoreType = {
    ...
    dispatchMessageToFeedBackMap: (name: string, newFeedBackMessage: BlazeFeedBackMessageType)=>void
}
...
export const useBlazeFeedBackStore = create<BlazeFeedBackStoreType>((set) => ({
  ...
  dispatchMessageToFeedBackMap: (name, newFeedBackMessage: BlazeFeedBackMessageType) => {
    set((state)=>{
        if(!state.feedBacksMessageMap){
            return state
        }
